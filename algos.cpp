...
std::list<int> walkableRadius(int stationIndex, double radius, std::vector<Station> & stationVec){
    ...
    for(int i = 0; i != stationVec.size(); i++){
        if(realDistance(stationIndex, stationVec[i]) <= radius && i != stationIndex){
...
std::vector<int> walkableNumber(int stationIndex, int x, std::vector<Station> & stationVec){
    ...
    for(int i = 0; i != x; i++){
        ...
        // not stationIndex
            ...
            walkable[i] = std::make_pair(i, realDistance(stationIndex,stationVec[i]));
            // check against longest
            if(realDistance(stationIndex,stationVec[i]) > walkable[longest].second){
                longest = i;
            }
        }
    }
    ...
    for(int i = x; i != stationsVec.size(); i++){
        if(i != stationIndex){
            // check against longest
            if( realDistance(stationIndex,stationVec[i]) > walkable[longest].second ){
                walkable[longest] = std::make_pair(i, realDistance(stationIndex,stationVec[i]));
