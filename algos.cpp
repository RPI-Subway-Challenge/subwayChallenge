...
int findLongest(std::vector<std::pair<int, double>> walkable)
{
    ...
    {
        if (walkable[i].second > walkable[longest].second)
        {
            longest = i;
        }
    }
...
std::vector<std::pair<int, double>> walkableNumber(int stationIndex, int x, std::vector<Station> &stationVec)
{
    ...
    if (x < 1)
    {
        std::cerr << "ERROR: X in walkableNumber must be >= 1\n";
        return walkable;
    }

    if (x >= stationVec.size())
    {
        std::cerr << "ERROR: X in walkableNumber must be < stationVec.size()\n";
        return walkable;
    }
...
std::vector<std::pair<int, std::string>> BFS(std::vector<Station> stations, int startingID){
    const int goal = 472; 
    std::queue<int> queue;
    std::vector<Station>::iterator StationItr;
    ...
    while (!(queue.empty())) {
      ...
      queue.pop();
      StationItr = stations.begin();
      for (StationItr; StationItr != stations[u].end(); StationItr++) {
         if (status[*StationItr.id] == 0) { //0 is unvisited node
            status[*StationItr] = 1; //1 is visited but not completed
            dist[*StationItr] = dist[u] + 1;
            par[*i] = u;
            q.push(*i);
         }
      }
      status[u] = "black"; //black is completed node
    }
}
